	...
	requestRedraw();
		  if (progress <= animationThermoLength) { raf = window.requestAnimFrame(drawAll);}
	...
	function generateThermosContainerHtml(){		
		
		return '<div id="tempwidget" class="col s12 m6 ">'+	
			'<div id="dashtemp" class="card-panel center no-select row">'+
			'<div id="dashtemptitle"  class="flow-text center textOnPrimaryColor dashTitle primaryLightColor">Températures</div>'+
			'<div id="dashtempcontent"  class="center dashContent">'+
				spinner()+
			'</div></div></div>';		
	}	
